...
def test_autoriz_valid_email_pass(selenium):
    """TC-RT001 Тест авторизации с валидными e-mail и пароль."""
...
def test_autoriz_invalid_email_pass(selenium, incor_email, incor_passw):
    """TC-RT002,003 "Проверка входа пользователя с невалидными email и паролем:
...
def test_elements_of_autoriz(selenium):
    """TC-RT004 Проверка раздела "Авторизация" на наличие основных элементов."""
    page = AuthPage(selenium)
...
def test_menu_type_autoriz(selenium):
    """TC-RT005 Проверка названия табов в меню выбора типа авторизации."""
    try:
        page = AuthPage(selenium)
...
def test_menu_of_type_active_autoriz(selenium):
    """TC-RT006 Проверка выбора таба по умолчанию в меню выбора типа авторизации."""
    page = AuthPage(selenium)
...
def test_placeholder_name_swap(selenium):
    """TC-RT007 Тест смены полей ввода при смене типа авторизации."""
    page = AuthPage(selenium)
...
def test_forgot_password_link(selenium):
    """TC-RT008 Переход к форме "Восстановление пароля"."""
    page = AuthPage(selenium)
...
def test_registration_link(selenium):
    """TC-RT009 Переход к форме "Регистрация"."""
    page = AuthPage(selenium)
...
def test_page_logo_registration(selenium):
    """TC-RT010 Проверка блока с логотипом/названием Компании на странице "Регистрация"."""
    try:
...
def test_elements_registration(selenium):
    """TC-RT011 Проверка формы "Регистрация" на наличие основных элементов."""
    try:
        page_reg = RegistrPage(selenium)
...
def test_names_elements_registration(selenium):
    """TC-RT012 Проверка формы "Регистрация" на соответствие названий элементов блока требованию."""
    try:
        page_reg = RegistrPage(selenium)
...
def test_registration_valid_data(selenium):
    """TC-RT013 Проверка регистрации пользователя с валидными данными: "Имя" и "Фамилия" на кириллице."""
    page_reg = RegistrPage(selenium)
...
def test_registration_invalid_data(selenium):
    """TC-RT014 Проверка на уникальность введенного e-mail в форме "Регистрация"."""
    page_reg = RegistrPage(selenium)
    page_reg.first_name.send_keys(Settings.first_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.last_name)
...
def test_registration_and_redir_auth(selenium):
    """TC-RT015 Тест формы "Авторизация" после нажатия кнопки "Войти" при регистрации пользователя e-mail,
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    """TC-RT016 Тест поля ввода "Имя" формы "Регистрация" допустимыми валидными значениями:
...
def test_first_name_invalid_data(selenium, invalid_first_name):
    """TC-RT017 Тест поля ввода "Имя" формы "Регистрация" невалидными значениями:
    пустое значение;
...
def test_last_name_valid_data(selenium, valid_password):
    """TC-RT018 Тест поля ввода "Пароль" формы "Регистрация" валидными значениями.:
...
def test_registration_confirm_password_valid_data(selenium):
    """TC-RT019 Тест поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация»
...
def test_registration_confirm_password_invalid_data(selenium):
    """TC-RT020 Тест поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация»
